...
module.exports = class ApiSupport {
  ...
  async tokenPostRequest(authorizationCode, redirectUri) {
    return await axios.post(this.baseUrl + "/token", `code=${authorizationCode}&grant_type=authorization_code&redirect_uri=${encodeURIComponent(redirectUri)}`, { headers: { "Content-Type": "text/plain" } });

  }

  async userInfoPostRequest(accessToken) {
    return await axios.post(this.baseUrl + "/userinfo", null, { headers: { "Authorization": `Bearer ${accessToken}` } });
  }
  ...
  async validateJwtToken(jwtToken) {
    ...
    expect(JSON.parse(decodeRawBody).vc.credentialSubject.name[0].nameParts[0].value).to.equal(userData.firstName);
    expect(JSON.parse(decodeRawBody).vc.credentialSubject.name[0].nameParts[1].value).to.equal(userData.middleName);
    expect(JSON.parse(decodeRawBody).vc.credentialSubject.name[0].nameParts[2].value).to.equal(userData.lastName);
    expect(JSON.parse(decodeRawBody).vc.credentialSubject.birthDate[0].value).to.equal(userData.dob);
